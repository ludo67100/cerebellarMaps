# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 15:03:27 2019

Computes linear regression between clusters 

from XX_Map_2D_Average_Amp_per_cluster_2.0.xlsx files
...
savedir= 'E:/000_PAPER/Amplitude_Analysis/Sigma/04_ZONES_CORRELATION/FIGURES/Norm Max'

saveDirIndPlots = 'E:/000_PAPER/Amplitude_Analysis/Sigma/04_ZONES_CORRELATION/Norm Max/FIGURES/00_Individual Plots'

conditions = ['WT','ENR1','ENR2','LC','LS','EC','ES']
colors = ['skyblue','limegreen','green','lightcoral','black','orange','purple']
cmaps = ['Blues','Greens','Greens','Reds','Greys','Oranges','Purples']
...
N = 8
r_min = 0.6 #Minimum r value for detection as correlation 
zscore = 3.09
...
for cond,color,cmap in zip(conditions,colors,cmaps) :
            ...
            for band_B, x_idx in zip(bands,range(len(bands))):
                ...
                if individual_plot == True : 
                    ...
                    if savefigindividualplots == True:
                        plt.savefig('{}/{}_{}_vs_{}_scatter.pdf'.format(saveDirIndPlots,cond,band_A, band_B))
...
df = pd.DataFrame(SORTED_COEFFS, index=conditions, columns=R_labels)

if savedata == True : 
    
    df.to_excel('{}/00_SORTED_COEFFICIENTS_MICROZONES.xlsx'.format(savedir))