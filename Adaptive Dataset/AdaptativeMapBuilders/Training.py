# -*- coding: utf-8 -*-
"""
Created on Mon Jan 13 12:35:20 2020
...
UPDATE 06/05/2019 : This version runs with the last version of neo 
...
#--------------ADAPT DIRECTORIES AND FILE LOCATION IF NEEDED-------------------

#The folder containing the raw data (condition)
datadir = '_Adaptive_Dataset/ENR'
...
#List of experiments to get
NAMES = [
        '160517(1)','160517(2)','160517(3)',
        '170517(1)','170517(2)',
        '180517(1)','180517(2)','180517(3)',
        '190517(1)','190517(2)',
        '210319(2)','210319(3)',
        '220319(1)','220319(2)',
        '230319(1)','230319(2)',
        '171219(1)','171219(2)',
        'ENR_270820(1)','ENR_270820(2)',
        'ENR_280820(1)',
        'ENR_090920(1)','ENR_090920(2)',
        'ENR_100920(1)']

#List of corresponding data folders
FOLDERS = [
        'ENR 16-05-2017','ENR 16-05-2017','ENR 16-05-2017',
        'ENR 17-05-2017','ENR 17-05-2017',
        'ENR 18-05-2017','ENR 18-05-2017','ENR 18-05-2017',
        'ENR 19-05-2017','ENR 19-05-2017',
        'ENR 21-03-2019','ENR 21-03-2019',
        'ENR 22-03-2019','ENR 22-03-2019',
        'ENR 23-03-2019','ENR 23-03-2019',
        '17112019(0)','17112019(0)',
        '27082020(0)','27082020(0)',
        '28082020(4)',
        '09092020(3)','09092020(3)',
        '10092020(II)']

SAVING = True #If True, saves the data in savedir
CLOSE_FIG = False #If true, will close figures after computing 
...
df  = pd.read_excel(measureFile,
                    sheet_name='ENR',index_col=0,header=1)  #DataFrame containing zebrin band file 

#Excel sheet with files and parameters : GC-PC_mappings_data_info.xlsx  
filelist  = pd.read_excel(infoFile,
                          sheet_name='Training (ENR)',index_col=0,header=0)  #DataFrame containing zebrin band file 

#Number of bins for noise fitting
number_bin_fit = 30
...
for NAME,FOLDER,IDX in zip(NAMES,FOLDERS,range(len(NAMES))):
    ...
    try : 
        ...
        name = NAME
        manip = NAME.split('_')[1] #For WT nomenclature only 
        ...
        for scanspot in range(files.shape[0]):  
            ...
            if protocol == 'short 96':
                ...
                noise_baseline_begin = np.where(time>=0.10)[0][0]
                noise_baseline_end = np.where(time>=0.15)[0][0]
                
                win_baseline_begin = np.where(time>=0.24)[0][0]
                win_baseline_end = np.where(time>=0.25)[0][0]        
                
                window_begin = np.where(time>=0.25)[0][0]
                window_end = np.where(time>=0.45)[0][0]
                
                noise_begin = np.where(time>=0.15)[0][0]
                noise_end = np.where(time>=0.24)[0][0]
        ...
        #--------------------------------2D ZSCORE-------------------------------------
        sigma_2D = np.full((6,64),np.abs(root_x))
