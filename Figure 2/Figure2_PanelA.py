...
computes figures for Figure 2 panel A
...
#Round factor, i.e. decimal to round for results
RF = 2
...
def effect_size(x,y):
    ...
    if np.nanmean(x) >= np.nanmean(y):
        es = (np.nanmean(x) - np.nanmean(y)) / np.nanstd(np.concatenate((x,y)))
    else:
        es =(np.nanmean(y) - np.nanmean(x)) / np.nanstd(np.concatenate((x,y)))
...
for group,index in zip(groups,range(len(groups))):
    ...
    print ('Avg Amplitude per mice at {} = {} +/- {}'.format(group, round(np.nanmean(AVG_MEASURE),RF),round(np.nanstd(AVG_MEASURE),RF)))
    print ('Med Amplitude per mice at {} = {} +/- {}'.format(group, round(np.nanmedian(AVG_MEASURE),RF),round(MAD(AVG_MEASURE),RF)))
    ...
    print ('Total (MEAN) currents per mice at {} = {} +/- {}'.format(group, round(np.nanmean(TOTAL_MEASURE),RF),round(np.nanstd(TOTAL_MEASURE),RF)))
    print ('Total (MEDIAN) currents per mice at {} = {} +/- {}'.format(group, round(np.nanmedian(TOTAL_MEASURE),RF),round(MAD(TOTAL_MEASURE),RF)))
    ...
    print ('Avg activation per mice at {} = {} +/- {}'.format(group, round(np.nanmean(AVG_PROP),RF),round(np.nanstd(AVG_PROP),RF)))
    print ('Med activation per mice at {} = {} +/- {}'.format(group, round(np.nanmedian(AVG_PROP),RF),round(MAD(AVG_PROP),RF)))
    ...
    #Avg values: 
    print ('Avg Amplitude at {} = {} +/- {}'.format(group, round(np.nanmean(distributions),RF),round(np.nanstd(distributions),RF)))
    print ('Med Amplitude at {} = {} +/- {}'.format(group, round(np.nanmedian(distributions),RF),round(MAD(distributions),RF)))
...
if statsToDo == True:
    ...
    print('KW test on all Amplitude distributions')
    
    allAmpKW = stats.kruskal(DISTRIBUTIONS[0],
                             DISTRIBUTIONS[1],
                             DISTRIBUTIONS[2],
                             DISTRIBUTIONS[3])
    
    print('KW stat value = {}'.format(allAmpKW[0]))
    print('KW p value = {}'.format(allAmpKW[1]))
    ...
    #Compute effect size vs P9 data
    print('EFFECT SIZE (based on mean)')
    
    for i in range(len(DISTRIBUTIONS)): 
        
        ref = DISTRIBUTIONS[0] #The CTRL data
        
        print('Effect size P9  vs {}: {}'.format(groups[i],effect_size(ref, DISTRIBUTIONS[i])))
        print('')
        
        
        
    #Compute effect size vs Adult Data data
    print('EFFECT SIZE (based on mean)')
    
    for i in range(len(DISTRIBUTIONS)): 
        
        ref = DISTRIBUTIONS[3] #The P30 data
        
        print('Effect size P30  vs {}: {}'.format(groups[i],effect_size(ref, DISTRIBUTIONS[i])))
        print('')
