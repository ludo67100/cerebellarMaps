...
for st in days:
    ...
    # Graph property dictionary: structure:  developmental stage -> "modularity" -> index of (ratnum,cellno) 
    graph_properties[st] = dict()
    graph_properties[st]["modularity"] = dict()
    graph_properties[st]["indices"] = []
    graph_properties[st]["names"] = []
    fig1.suptitle("Days:"+st,fontsize=15,fontweight='bold')  # Correlation map original
    fig2.suptitle("Days:"+st+" rearranged, gamma = "+str(gamma_re_arrange),fontsize=15,fontweight='bold') # Correlation map re-arranged after community detection 
    ...
    for i,(rn,cn) in enumerate(zip(data_slice["rat_num"],data_slice["cell_num"])):
        ...
        if str(cn) in list(data_2d[st][str(rn)].keys()):
            ...
            if mat_type == "norm":
                corr_2d = corr_2d
            cov_2d_dict[st][str(rn)][str(cn)]["corr"] = corr_2d 
            ...
            # Module degree zscore on the hemipshere resolution - ipsilateral and contralateral
            if len(data_2d[st][str(rn)][str(cn)]["ind_ipsi"]) > 0:
...
pickle.dump(graph_properties,open(data_target_dir+"graph_properties_days_"+mat_type+".pickle","wb"))
